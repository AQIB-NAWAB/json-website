...
export default function getFindResultsByGlobalRegExp (text: string, regex: RegExp, offset = 0): RegExpResult[] {
  let results: RegExpResult[] = []
  let regexResult
  if (Array.isArray(text)) throw new Error('array used for regular expression!')
  if (!regex.global) throw new Error(`regex for getFindResultsByGlobalRegExp() has no global flag ${regex.toString()}`)
  while ((regexResult = regex.exec(text)) !== null) {
    ...
    const groupsReverseMap: Record<string, string> = Object.entries((regexResult.groups || {}))
    ...
    const newResult: RegExpResult = {
      ...
      index: regexResult.index + (regexResult.groups?.pretarget?.length || 0) + offset,
      groups
    }
