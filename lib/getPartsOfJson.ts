...
export default function getPartsOfJson (serializedJson: string, offset = 0, jsonPath = '$'): SyntaxPart[] {
  ...
  if (objectMatch) {
    const parts: SyntaxPart[] = objectMatch.groups.reduce((acc: SyntaxPart[], objectMatchGroup: RegExpGroupResult) => {
      ...
      if (objectMatchGroup.name === 'objectContent') {
        const parts = getPartsOfJsonObjectContent(objectMatchGroup.match, totalGroupOffset, jsonPath)
        return [...acc, ...parts]
      }
      return acc
    }, [] as SyntaxPart[])
  ...
  if (arrayMatch) {
    const parts: SyntaxPart[] = arrayMatch.groups.reduce((acc: SyntaxPart[], arrayMatchGroup: RegExpGroupResult) => {
      ...
      if (arrayMatchGroup.name === 'arrayContent') {
        const parts = getPartsOfArrayContent(arrayMatchGroup.match, totalGroupOffset, jsonPath)
        return [...acc, ...parts]
      }
      return acc
    }, [] as SyntaxPart[])
...
const getPartsOfArrayContent = (serializedJson: string, offset = 0, jsonPath: string): SyntaxPart[] => {
  ...
  const partsFromArrayValues: SyntaxPart[] = arrayValues.reduce((acc: SyntaxPart[], arrayValue, index) => {
    const indexJsonPath = `${jsonPath}[${index}]`
    const value = serializedJson.substr(arrayValue.index, arrayValue.length)
    const parts: SyntaxPart[] = getPartsOfJson(value, offset + arrayValue.index, indexJsonPath)
    return [...acc, ...parts]
  }, [] as SyntaxPart[])
